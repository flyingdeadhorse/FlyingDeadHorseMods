    ...
    execute: async function() {
        ...
        if (!window.rmWoodTextureImg) {
            try {
                ...
                await new Promise((resolve) => {
                    img.onload = () => { window.rmWoodTextureImg = img; resolve(); };
                    img.onerror = () => { console.warn("Rollmate: Failed to load wood texture."); resolve(); };
                });
            } catch(e) {}
        }
        ...
        if (!window.rm3DLibsLoaded) {
            ...
            try {
                ...
                return console.error("Rollmate: Fehler beim Laden der 3D-Bibliotheken.");
            }
        }
        ...
        if (!window.rmUniversalSceneControlsHook) {
            Hooks.on("renderSceneControls", (app, html) => {
                if (html.find('.rollmate-main-btn').length === 0) {
                    const btn = $(`<li class="scene-control rollmate-main-btn" title="Rollmate Universal"><i class="fas fa-dice"></i></li>`);
                    btn.on('click', () => { window.RollmateUniversal.execute(); }); html.append(btn); 
                }
            }); window.rmUniversalSceneControlsHook = true; ui.controls.render(); 
        }
        ...
        window.rmAddActorsToCombat = async function(actorIds) {
            ...
            for (let id of actorIds) {
                ...
                if (token && !game.combat.combatants.find(c => c.actorId === id)) {
                    toAdd.push({ tokenId: token.id, actorId: id, hidden: token.document.hidden });
                }
            }
            if (toAdd.length > 0) await game.combat.createEmbeddedDocuments("Combatant", toAdd);
        };
        ...
        if (!window.rmUniversalSceneHook) {
            Hooks.on("updateScene", (scene, data) => {
                if (hasProperty(data, `flags.${RollmateFlags.scope}.-=${RollmateFlags.checkKey}`)) cleanUp();
                if (hasProperty(data, `flags.${RollmateFlags.scope}.${RollmateFlags.checkKey}`)) {
                    if (!game.user.isGM && $('#rollmate-universal-overlay').length === 0) { window.RollmateUniversal.execute(); }
                }
                if (hasProperty(data, `flags.${RollmateFlags.scope}.roulettePhase`)) {
                    const phase = getProperty(data, `flags.${RollmateFlags.scope}.roulettePhase`);
                    const winnerId = canvas.scene.getFlag(RollmateFlags.scope, 'rouletteWinner');
                    if (window.handleRoulettePhase) window.handleRoulettePhase(phase, winnerId);
                }
        ...
        if (!window.rmUniversalActorHook) {
            Hooks.on("updateActor", (actor, changes, options, userId) => {
                if (hasProperty(changes, `flags.${RollmateFlags.scope}.${RollmateFlags.resultKey}`)) {
                    const result = actor.getFlag(RollmateFlags.scope, RollmateFlags.resultKey);
                    if (result && $('#rollmate-universal-overlay').length > 0) {
                        if (result.status === "rolling") {
                            if (typeof window.resetCardForReroll === 'function') window.resetCardForReroll(actor.id);
                            ...
                            if (game.user.id !== userId) {
                                if (typeof window.initWebGLDiceFunc === 'function') {
                                    ...
                                    setTimeout(() => {
                                        if (window.rmActive3DDice && window.rmActive3DDice[actor.id]) {
        ...
        function cleanUp() { 
            ...
            if(window.rmFloatAnimFrame) { cancelAnimationFrame(window.rmFloatAnimFrame); window.rmFloatAnimFrame = null; }
        ...
        const getSysConfig = () => {
            if (activeSystem === 'pf2e') return { id: 'pf2e', name: 'Pathfinder 2', themeColor: '#FFD700', glowColor: 'rgba(255,215,0,0.6)', logoImg: RollmateAssets.images.pf2Logo };
            if (activeSystem === 'dnd5e') return { id: 'dnd5e', name: 'Dungeons and Dragons 5', themeColor: '#FF5722', glowColor: 'rgba(255, 87, 34, 0.6)', logoImg: RollmateAssets.images.dnd5Logo };
        };
        let sysCfg = getSysConfig();

        // --- CUSTOM DESIGN flags ---
        const gmUser = game.users.find(u => u.isGM && u.active) || game.users.find(u => u.isGM) || game.user;

        let userBgImg = gmUser.getFlag(flagScope, 'artistBgImg') ?? RollmateAssets.images.defaultBg;
        ...
        let userBorderTrans = savedBorderTrans === false ? userBorder : 'transparent';
        ...
        let userParticleColor = gmUser.getFlag(flagScope, 'artistParticleColor') ?? '#0088ff'; 
        ...
        let userCardImg = gmUser.getFlag(flagScope, 'artistCardImg') ?? RollmateAssets.images.defaultCard;
        
        let userBoardMode = gmUser.getFlag(flagScope, 'artistBoardMode') ?? 'immersive'; 
        ...
        const activeCheckInstance = canvas.scene.getFlag(RollmateFlags.scope, RollmateFlags.checkKey);
        if (!isGM) {
            if (!activeCheckInstance) return console.warn("Rollmate: Keine aktive Würfelrunde gefunden!");
            if (typeof buildStage === 'function' && $('#rollmate-universal-overlay').length === 0) buildStage(activeCheckInstance);
        } else {
...
function initRollmateHooks() {
    $(document).off('click.rmOpen').on('click.rmOpen', '.open-rollmate-btn', function(e) {
        e.preventDefault();
    ...
    if (!window.rollmateAutoOpenActive) {
        Hooks.on("createChatMessage", (message) => {
            if (message.content && message.content.includes("open-rollmate-btn")) {
                ...
                setTimeout(() => { window.RollmateUniversal.execute(); }, 500);
            }
        });
        ...
        Hooks.on("controlToken", (token, controlled) => {
            ...
            if (controlled) {
                if (card.length === 0) {
                    ...
                    let hiddenHtml = isHidden ? `<i class="fas fa-eye-slash" style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); font-size:4em; color:rgba(255,255,255,0.7); text-shadow:0 0 15px black; z-index:30; pointer-events:none;"></i>` : '';
    ...
    if (game.user.isGM) {
        ...
        if ($(`#${launcherId}`).length === 0) {
            ...
            if (!savedPos || isNaN(savedPos.top) || isNaN(savedPos.left)) {
                savedPos = { top: window.innerHeight / 2 - 25, left: window.innerWidth / 2 - 25 };
            } else {
                savedPos.left = Math.max(0, Math.min(savedPos.left, window.innerWidth - 65));
                savedPos.top = Math.max(0, Math.min(savedPos.top, window.innerHeight - 65));
            }
            ...
            $(window).on('mouseup.rmLauncher', async function(e) {
                ...
                if (isDragging && hasMoved) { 
                    hasMoved = false;
                    isDragging = false;
                    await game.user.setFlag(RollmateFlags.scope, posKey, { left: parseInt(btn.css('left')), top: parseInt(btn.css('top')) }); 
                } 
